...
static unsigned char *minimp4_vector_alloc_tail(minimp4_vector_t *h, int bytes)
{
    ...
    {
    	printf("minimp4_vector_init fail\n");
        return NULL;
    }
    if ((h->capacity - h->bytes) < bytes && !minimp4_vector_grow(h, bytes))
    {
    	printf("minimp4_vector_grow fail\n");
        return NULL;
    }
    assert(h->data);
...
int MP4E_put_sample(MP4E_mux_t *mux, int track_num, const void *data, int data_bytes, int duration, int kind)
{
    ...
    if (kind != MP4E_SAMPLE_CONTINUATION)
    {
        ...
        {
        	printf("add_sample_descriptor fail\n");
            return MP4E_STATUS_NO_MEMORY;
        }
    } else
    {
    ...
    if (mux->sequential_mode_flag)
    {
        ...
        {
        	printf("minimp4_vector_put: MP4E_STATUS_NO_MEMORY\n");
            return MP4E_STATUS_NO_MEMORY;
        }
    } else
    {
...
static void h264e_bs_flush(bs_t *bs)
{
    //*bs->buf = SWAP32(bs->cache);
	uint32_t a = bs->cache;
	uint32_t p1 = (a >> 24) & 0x000000FF;
	uint32_t p2 = (a >> 8) &  0x0000FF00;
	uint32_t p3 = (a << 8) &  0x00FF0000;
	uint32_t p4 = (a << 24) & 0xFF000000;
	*bs->buf = p4|p3|p2|p1;
}
...
static int transcode_nalu(h264_sps_id_patcher_t *h, const unsigned char *src, int nalu_bytes, unsigned char *dst)
{
    ...
    memset(&bst[0],0,sizeof(bit_reader_t));
    memset(&bs[0],0,sizeof(bit_reader_t));
    memset(&bdt[0],0,sizeof(bs_t));
    memset(&bd[0],0,sizeof(bs_t));
...
int mp4_h26x_write_nal(mp4_h26x_writer_t *h, const unsigned char *nal, int length, unsigned timeStamp90kHz_next)
{
    ...
    for (;;nal++)
    {
        ...
        if (!nal1)
        {
        	printf("Nal1: MP4E_STATUS_NO_MEMORY\n");
            return MP4E_STATUS_NO_MEMORY;
        }
        nal2 = (unsigned char *)malloc(sizeof_nal*17/16 + 32);
        if (!nal2)
        {
        	printf("Nal2: MP4E_STATUS_NO_MEMORY\n");
            free(nal1);
            return MP4E_STATUS_NO_MEMORY;
        }
...
exit_with_free:
        ...
        case 7:
        	...
        	MP4E_set_sps(h->mux, h->mux_track_id, nal, size);
            h->need_sps = 0;
            break;
        case 8:
            if (h->need_sps)
            ...
            MP4E_set_pps(h->mux, h->mux_track_id, nal, size);
            h->need_pps = 0;
            break;
        case 5:
            if (h->need_sps)
        ...
        default:
            ...
            if (!h->need_pps && !h->need_idr)
            {
                ...
                //unsigned slice_type = ue_bits(bs);
                int sample_kind = MP4E_SAMPLE_DEFAULT;
                nal2[0] = (unsigned char)((size) >> 24);
                nal2[1] = (unsigned char)((size) >> 16);
                nal2[2] = (unsigned char)((size) >>  8);
                nal2[3] = (unsigned char)((size));
                first_mb_in_slice = 0;
                ...
                memcpy(&nal2[4],nal,size);
                err = MP4E_put_sample(h->mux, h->mux_track_id, nal2, size + 4, timeStamp90kHz_next, sample_kind);
            }
            break;
        }
        free(nal1);
